...
/******************************** Configuration of CR2******************************************/
	...
	{
		pUSARTHandle->pUSARTx->USART_CR2 |= (0 << USART_CR2_STOP);
	}
...
/******************************** Configuration of BRR(Baudrate register)******************************************/
	...
	//Implement the code to enable the peripheral for given USART peripheral
	USART_PeripheralControl(pUSARTHandle->pUSARTx, Enable);
	...
	for(uint32_t i = 0 ; i < Len; i++)
	{
		...
		else
		{
			...
			else
			{
				 ...
				 *pRxBuffer = (pUSARTHandle->pUSARTx->USART_RDR  & (uint8_t)0x7F);
...
{
	uint8_t temp ;
	temp = (pUSARTx->USART_ISR) & (1<<StatusFlagname);
	return temp;
}
...
{
	pUSARTx->USART_ICR |= (1<< StatusFlagName);
}
...
//IRQPeriortyconfig
void USART_IRQInterruptPeriority(uint8_t IRQNumber, uint32_t IRQPeriority)
{
	uint8_t iprx = IRQNumber / 4;
	uint8_t iprx_section = IRQNumber % 4;
	uint8_t shift_number = (8 * iprx_section) + (8 - NO_PR_BITS_IMPLEMENTED);
	*(NVIC_PR_BASE_ADDR + (iprx) ) |= ( IRQPeriority << shift_number);
	...
	if(temp1 && temp2 )
	{
		...
		if ( pUSARTHandle->TxState == USART_BUSY_IN_TX)
		{
			...
			if(! pUSARTHandle->TxLen )
			{
				...
				pUSARTHandle->pUSARTx->USART_ICR |= ( 1 << USART_ICR_TCCF);

				//Implement the code to clear the TCIE control bit
				pUSARTHandle->pUSARTx->USART_CR1 &= ~( 1 << USART_CR1_TCIE);

				//Reset the application state
				pUSARTHandle->TxState = USART_BUSY_IN_READY;
	...
	if(temp1 && temp2 )
	{
		...
		if(pUSARTHandle->RxState == USART_BUSY_IN_RX)
		{
			...
			if(! pUSARTHandle->RxLen)
			{
				//disable the rxne
				pUSARTHandle->pUSARTx->USART_CR1 &= ~( 1 << USART_CR1_RXNEIE );
				pUSARTHandle->RxState = USART_BUSY_IN_READY;
				USART_ApplicationEventCallback(pUSARTHandle,USART_EVENT_RX_COMPLT);
			}
		}
	}
...
*/
	...
	temp3 = pUSARTHandle->pUSARTx->USART_ISR & ( 1 << USART_ISR_CTSIF);
	...
	if(temp2  && temp3 )
	{
		...
		pUSARTHandle->pUSARTx->USART_ICR |= (1 << USART_ICR_CTSCF);
...
/*************************Check for IDLE detection flag ********************************************/
	...
	if(temp1 && temp2)
	{
		...
		pUSARTHandle->pUSARTx->USART_ICR |= (1 << USART_ICR_IDLECF);
...
/*************************Check for Overrun detection flag ********************************************/
	...
	if(temp1  && temp2 )
	{
		...
		pUSARTHandle->pUSARTx->USART_ICR |= (1 << USART_ICR_ORECF);
